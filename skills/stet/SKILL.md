...
- Keep the treatment mechanism explicit: file overlays for `agents_md`,
  `claude_md`, `claude_rules`, `skill_diff`, and `harness_bundle`;
  prompt-template context for `docs_glob`; runtime selector for
  `model_update`. `claude_rules` is Claude-only (forces both arms to
  `claude-code`) and A/B tests `.claude/rules/**/*.md`.
