...
unsupported on the worktree path. After each task persists its artifacts, Stet
retires that task's isolated worktrees, object store, homes, and owned build
caches; `--worktree-keep` protects them instead, and the task artifact directory
retains the lifecycle receipt. On worktree, Stet resolves the baseline
...
The integrity scanner recognizes syntactically valid Bazel/Bazelisk target
patterns in parser-proven target positions within direct or simply composed
commands; path-valued flags, redirect operands, post-`--` arguments, and
unsupported shell grammar remain fail-closed.
