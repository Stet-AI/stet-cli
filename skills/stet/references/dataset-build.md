...
derivation abstains (query pattern failure, unsafe/out-of-pattern label, budget overflow, or no covering target), the build skips the task unless a manual fixed-manifest build explicitly sets `test_selector.fallback: keep_broad`. Only that explicit mode may rerun the original broad command for base+gold under the same multi-run flake policy, and it accepts only a physical broad base-fail/gold-pass recorded as `proof_strength: proven_dynamic_f2p`; `abstained_kept_broad` alone is not execution proof. Stet never falls back to an incomplete label subset. Overflow recovery may use `--keep_going` only for source-rdeps after every changed source is strictly proven; a typed transient `ExternalDepsException`/exit-37 query receives exactly one same-task retry. If that retry is exhausted during selector work, the task is rejected with `runtime_classification: query_retry_exhausted` and manifest canary selection continues; timeout, cancellation, auth/toolchain failure, and other shared infrastructure failures still fail closed. Query attempt provenance records exit code, complete/partial mode, elapsed time, redacted diagnostics, and digests. `--bazel-candidate-budget` / `build.bazel_candidate_budget` bounds the same full candidate set in pre-material selection and verification (default 16, maximum 4096), while `--bazel-dynamic-attempt-budget` / `build.bazel_dynamic_attempt_budget` separately bounds dynamic attempts; neither path truncates or samples first-N labels. Infrastructure failures (disk/extraction errors, missing toolchain binaries, selector crashes) classify as `executor_runtime_error`, skip the task instead of minting a fallback proof, and are never recorded as `f2p_failed`.
